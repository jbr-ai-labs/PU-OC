...
class RandomResizedCropLayer(nn.Module):
    ...
    def forward(self, inputs, whbias=None, **kwargs):
        ...
        grid = F.affine_grid(_theta, inputs.size(), align_corners=True, **kwargs).to(_device)
        output = F.grid_sample(inputs, grid, padding_mode='reflection', align_corners=True, **kwargs)

        if self.size is not None:
            output = F.adaptive_avg_pool3d(output, self.size)
...
class HorizontalFlipRandomCrop(nn.Module):
    ...
    def forward(self, input, sign=None, bias=None, rotation=None, **kwargs):
        ...
        grid = F.affine_grid(_theta, input.size(), align_corners=True, **kwargs).to(_device)
        output = F.grid_sample(input, grid, padding_mode='reflection', align_corners=True, **kwargs)
...
class HorizontalFlipLayer(nn.Module):
    ...
    def forward(self, inputs, **kwargs):
        ...
        _theta[:, 0, 0] = r_sign
        grid = F.affine_grid(_theta, inputs.size(), align_corners=True, **kwargs).to(_device)
        inputs = F.grid_sample(inputs, grid, padding_mode='reflection', align_corners=True, **kwargs)
